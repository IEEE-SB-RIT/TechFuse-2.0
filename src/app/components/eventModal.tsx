...
export default function EventModal({data, onClose}: EventModalProps) {
    ...
    return (
        <div
            ...
            <div
                className={`relative flex flex-col md:flex-row gap-6  gap-5 w-3xl max-w-full max-h-[90vh] overflow-y-auto   bg-white/10 backdrop-blur-xl border border-white/20 shadow-xl rounded-2xl p-6 transition-all duration-300 ease-in-out ${show ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}
                ref={modalRef}>
                {onClose && (
                    <button
                        onClick={closeModal}
